...
from src.backend.text_analysis.text_analyzer import (
    TextAnalyzer,
    TextAnalysisConfig,
    create_analysis_models,
)
from src.backend.text_analysis.base_models import Feature, Intention, Definition
...
from tests.conftest import sample_case_model, sample_text_analysis_config, sample_llm_config, temp_runtime_directory


class TestCreateAnalysisModels:
    """Tests pour create_analysis_models()"""
    
    def test_create_analysis_models_fr(self):
    ...
    def test_create_analysis_models_with_highlight(self):
    ...
    def test_create_analysis_models_multiple_features(self):
        """Test création avec plusieurs features"""
        locale = "fr"
        features = [
            Feature(id="nom", label="Nom", type=str, description="Nom", highlight_fragments=False),
            Feature(id="prenom", label="Prénom", type=str, description="Prénom", highlight_fragments=True),
        ]
...
class TestTextAnalyzerInit:
    ...
    def test_init_creates_features(self, sample_case_model, sample_text_analysis_config, temp_runtime_directory):
    ...
    def test_init_excludes_do_not_extract(self, sample_text_analysis_config, temp_runtime_directory):
    ...
    def test_init_includes_extract_and_highlight(self, sample_text_analysis_config, temp_runtime_directory):
...
class TestTextAnalyzerAnalyze:
    ...
    @pytest.fixture
    def analyzer(self, sample_case_model, sample_text_analysis_config, temp_runtime_directory):
    ...
    def test_analyze_with_json_schema(self, mock_llm_class, analyzer, sample_llm_config):
        """Test analyze() avec response_format_type='json_object'"""
        with patch('src.backend.text_analysis.text_analyzer.short_hash', return_value="test_hash"):
            with patch('src.backend.text_analysis.text_analyzer.get_cache_file_path') as mock_get_cache:
    ...
    @patch('src.backend.text_analysis.text_analyzer.LlmOpenAI')
    def test_analyze_with_pydantic_model(self, mock_llm_class, analyzer):
        ...
        with patch('src.backend.text_analysis.text_analyzer.short_hash', return_value="test_hash"):
            with patch('src.backend.text_analysis.text_analyzer.get_cache_file_path') as mock_get_cache:
                ...
                mock_result = Mock()
                mock_result.model_dump.return_value = {
                    "scorings": [{"intention_id": "intention1", "score": 5, "justification": "Test"}],
                    "nom": None
                }
    ...
    def test_analyze_with_cache(self, mock_get_cache_path, mock_hash, analyzer, sample_llm_config):
        ...
        cache_path = os.path.join(analyzer.runtime_directory, "cache", "cache_test_app_fr_test_hash.json")
    ...
    @patch('src.backend.text_analysis.text_analyzer.short_hash')
    @patch('src.backend.text_analysis.text_analyzer.get_cache_file_path')
    def test_analyze_cache_missing_file(self, mock_get_cache_path, mock_hash, analyzer, sample_llm_config):
        ...
        with patch('src.backend.text_analysis.text_analyzer.LlmOpenAI') as mock_llm_class:
            mock_llm_instance = Mock()
            mock_result = Mock()
    ...
    def test_analyze_unsupported_llm(self, analyzer):
    ...
    def test_analyze_with_ollama(self, mock_llm_class, analyzer):
    ...
    def test_analyze_with_scaleway(self, mock_llm_class, analyzer):
    ...
    @patch('src.backend.text_analysis.text_analyzer.LlmOpenAI')
    def test_analyze_adds_intention_other(self, mock_llm_class, analyzer, sample_llm_config):
        ...
        with patch('src.backend.text_analysis.text_analyzer.short_hash', return_value="test_hash"):
            with patch('src.backend.text_analysis.text_analyzer.get_cache_file_path') as mock_get_cache:
                ...
                mock_result.model_dump.return_value = {
                    "scorings": [
                        {"intention_id": "intention1", "score": 5, "justification": "Test"}
                    ],
                    "nom": None
                }
                mock_llm_instance.call_llm_with_json_schema.return_value = mock_result
                ...
                other_scoring = next((s for s in scorings if s.get("intention_id") == "other"), None)
...
class TestTextAnalyzerBuildPrompt:
    ...
    def test_build_prompt_markdown(self, analyzer, sample_llm_config):
    ...
    def test_build_prompt_text(self, analyzer):
    ...
    def test_build_prompt_includes_intentions(self, analyzer, sample_llm_config):
    ...
    def test_build_prompt_includes_definitions(self, analyzer, sample_llm_config):
