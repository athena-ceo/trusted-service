...
def ruleflow(input: CaseHandlingDecisionInput, output: CaseHandlingDecisionOutput):
    ...
    def package_cas_nominal_92(input: CaseHandlingDecisionInput, output: CaseHandlingDecisionOutput):
        def rule_depot_de_demande_d_asile_92(input: CaseHandlingDecisionInput, output: CaseHandlingDecisionOutput):
            if input.intention_id == "depot_de_demande_d_asile":
                output.details.append("rule_depot_de_demande_d_asile_92")  # Rule trace
                text = "**Les services de l'État dans les Hauts-de-Seine** - Asile"
                url = "https://www.hauts-de-seine.gouv.fr/index.php/Demarches/Etrangers-dans-les-Hauts-de-Seine/ASILE/Depot-d-une-demande-d-asile"
                output.acknowledgement_to_requester = f"#VISIT_PAGE,{text},{url}"
        ...
        def rule_rdv_remise_de_titre_92(input: CaseHandlingDecisionInput, output: CaseHandlingDecisionOutput):
            if input.intention_id == "rdv_remise_de_titre":
                text = "**Prendre un rendez-vous pour remise de titre** - Les services de l'État dans les Hauts-de-Seine"
                if input.field_values.get("arrondissement") == "ANT":
                    url = "https://www.hauts-de-seine.gouv.fr/Demarches/Etrangers-dans-les-Hauts-de-Seine/SEJOUR/SEJOUR-sous-prefecture-d-Antony/Remise-de-titre"
                elif input.field_values.get("arrondissement") == "BOULO":
                    url = "https://www.hauts-de-seine.gouv.fr/Demarches/Etrangers-dans-les-Hauts-de-Seine/SEJOUR/SEJOUR-sous-prefecture-de-Boulogne/Remise-de-titre"
                else:
                    url = "https://www.hauts-de-seine.gouv.fr/Demarches/Etrangers-dans-les-Hauts-de-Seine/SEJOUR/SEJOUR-prefecture-de-Nanterre/Remise-de-titre"
                output.details.append("rule_rdv_remise_de_titre_92")
                output.acknowledgement_to_requester = f"#VISIT_PAGE,{text},{url}"
                output.priority = "LOW"
        ...
        def rule_rdv_renouvellement_titre_sejour_hors_anef(input: CaseHandlingDecisionInput, output: CaseHandlingDecisionOutput):
            if input.intention_id == "rdv_renouvellement_titre_sejour_hors_anef":
                ...
                if input.field_values["statut"] in ["étudiant", "visiteur", "conjoint_de_français", "parent_d_enfant_français_mineur", "citoyen_UE"]:
                    ...
                    text = "**Prendre un rendez-vous** - Les services de l'État dans les Hauts-de-Seine"
                    url = "https://www.hauts-de-seine.gouv.fr/Demarches/Etrangers-dans-les-Hauts-de-Seine#!/Particuliers/page/N20306"
                    output.priority = "MEDIUM"
                    output.acknowledgement_to_requester = f"#VISIT_PAGE,{text},{url}"
                else:
                    output.priority = "HIGH"
        ...
        print(f"------- 📦 Executing package_cas_nominal_92")

        rule_depot_de_demande_d_asile_92(input, output)
        rule_refugie_ou_protege_subsidiaire(input, output)
        rule_expiration_d_une_atda(input, output)
        rule_expiration_d_une_api_cas_nominal(input, output)
        rule_expiration_d_une_api_api_expiree(input, output)
        ...
        rule_difficulte_prise_rdv(input, output)
        rule_rdv_sauf_conduit(input, output)
        rule_rdv_remise_de_titre_92(input, output)
        rule_rdv_renouvellement_recepisse(input, output)
        rule_rdv_renouvellement_titre_sejour_hors_anef(input, output)
        rule_dem_en_rapport_dublin_en_cours(input, output)
    ...
    def package_reajustements_92(input: CaseHandlingDecisionInput, output: CaseHandlingDecisionOutput):
        def rule_risque_sur_l_emploi(input: CaseHandlingDecisionInput, output: CaseHandlingDecisionOutput):
            if input.field_values["mention_de_risque_sur_l_emploi"]:
                output.details.append("rule_risque_sur_l_emploi")
                augmenter_niveau_priorite(output)

        print(f"------- 📦 Executing package_reajustements_92")
    ...
    elif input.field_values["departement"] == "92":
        package_cas_nominal_92(input, output)
        package_reajustements_92(input, output)
