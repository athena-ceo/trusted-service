...
class DecisionCenterExtract(BaseModel):
    ...
    def read_from_file(path) -> DecisionCenterExtract:
        extract: DecisionCenterExtract = DecisionCenterExtract.parse_file(
            path,
            encoding="utf-8",
        )
        return extract
    ...
    def create(
        ...
        dc_api_connection: Connection,
    ) -> DecisionCenterExtract:
        params = {"q": f"name:{decision_service_name}"}
        response_json = dc_api_connection.call_request(
            method="GET",
            endpoint="decisionservices",
            params=params,
        )

        if len(response_json["elements"]) == 0:
            msg = f'Decision service "{decision_service_name}" was not found'
            raise Exception(msg)
    ...
    def find_rule_or_dt_by_fully_qualified_name(self, fqn: str) -> RuleOrDT | None:
...
class DecisionService(Artefact):
    ...
    # @override
    def get_direct_children(self) -> list[Artefact]:
        return self.projects[:]  # Shallow copy
...
class Project(Artefact):
    ...
    def extract_rules_and_dts(self, dc_api_connection: Connection) -> None:
        ...
        all_rules_and_dts: list[RuleOrDT] = [
            RuleOrDT(
                id=r["id"],
                name=r["name"],
                fully_qualified_name=r["name"],
                rule_package=r["rulePackage"],
                html=r["html"],
                documentation=Documentation.parse_raw_documentation(
                    ensure_not_none(r["documentation"]),
                ),
            )
        ...
        for r in rules_and_dts:
            ...
            (content, format2) = Documentation.parse_raw_documentation(
                raw_documentation,
            )
    ...
    def compute_fully_qualified_names(self) -> None:
...
class Folder(Artefact):
    ...
    def propagate_down_fully_qualified_name(self) -> None:
        for sub_folder in self.sub_folders:
            sub_folder.fully_qualified_name = (
                self.fully_qualified_name + "." + sub_folder.name
            )
            sub_folder.propagate_down_fully_qualified_name()
        for rule_or_dt in self.rules_and_dts:
            rule_or_dt.fully_qualified_name = (
                self.fully_qualified_name + "." + rule_or_dt.name
            )
...
class Documentation(BaseModel):
    ...
    @staticmethod
    def parse_raw_documentation(
        raw_documentation: str,
    ) -> Documentation:  # returns (content, format)
        ...
        # Check for errors
        if not begin_found or not end_found or format_line is None:
...
def main() -> None:
    ...
    decision_center_extract0 = DecisionCenterExtract.create(
    ...
    for fully_qualified_name in [
        ...
        rule_or_dt = decision_center_extract.find_rule_or_dt_by_fully_qualified_name(
