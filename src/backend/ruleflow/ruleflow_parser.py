...
class RuleNode:
    ...
    output_assignments: List[OutputAssignment] = field(default_factory=list)  # Paramètres output structurés
...
class RuleflowParser:
    ...
    def _extract_helper_functions(self) -> List[str]:
        ...
        for node in ast.walk(self.ast_tree):
            if isinstance(node, ast.FunctionDef):
                ...
                if (node.name not in ruleflow_names and 
                    not self._is_in_class(node) and 
                    node.name not in excluded_names and
                    not node.name.startswith('package_') and
                    not node.name.startswith('rule_')):
                    start = node.lineno - 1
                    end = node.end_lineno
    ...
    def _extract_rules_from_package(self, package_func: ast.FunctionDef) -> List[RuleNode]:
        ...
        for rule_info in rule_calls:
            ...
            if rule_def:
                ...
                free_code, output_assignments = self._analyze_rule_code(rule_def, rule_code)
    ...
    def _analyze_rule_code(self, rule_func: ast.FunctionDef, rule_code: str) -> tuple[str, List[OutputAssignment]]:
        """
        ...
        self._extract_output_assignments_recursive(rule_func.body, output_assignments, rule_func.lineno)
    ...
    def _extract_output_assignments_recursive(self, stmts: List[ast.stmt], assignments: List[OutputAssignment], base_line: int):
        ...
        for stmt in stmts:
            if isinstance(stmt, ast.Assign):
                ...
                for target in stmt.targets:
                    if isinstance(target, ast.Attribute) and isinstance(target.value, ast.Name) and target.value.id == 'output':
                        try:
                ...
                if (isinstance(call.func, ast.Attribute) and 
                    ...
                    call.func.value.value.id == 'output' and
                ...
                self._extract_output_assignments_recursive(stmt.body, assignments, base_line)
                # Analyser récursivement la partie else
                self._extract_output_assignments_recursive(stmt.orelse, assignments, base_line)
                ...
                for handler in stmt.handlers:
                    self._extract_output_assignments_recursive(handler.body, assignments, base_line)
                self._extract_output_assignments_recursive(stmt.orelse, assignments, base_line)
                self._extract_output_assignments_recursive(stmt.finalbody, assignments, base_line)
    
    def _generate_free_code(self, rule_code: str, output_assignments: List[OutputAssignment]) -> str:
        """
        ...
        for i, line in enumerate(lines):
            ...
            if (stripped_line.startswith('output.') and 
                '=' in stripped_line and 
                not stripped_line.startswith('if ') and
                not stripped_line.startswith('elif ') and
                not line.strip().startswith('# ')):
                continue
            
            # Ignorer les appels directs à output.X.append (hors conditions)
            if (stripped_line.startswith('output.') and 
                '.append(' in stripped_line and
                not stripped_line.startswith('if ') and
                not stripped_line.startswith('elif ')):
                continue
...
class RuleflowGenerator:
    ...
    def _generate_rule(self, lines: List[str], rule: RuleNode, indent: int = 1):
        ...
        if hasattr(rule, 'free_code') and hasattr(rule, 'output_assignments') and rule.free_code:
    ...
    def _generate_rule_from_full_code(self, lines: List[str], rule: RuleNode, indent_str: str):
    ...
    def _generate_rule_from_components(self, lines: List[str], rule: RuleNode, indent_str: str):
        ...
        if hasattr(rule, 'output_assignments') and rule.output_assignments:
            ...
            for assignment in rule.output_assignments:
                if assignment.attribute.endswith('.append'):
                    ...
                    lines.append(f"{indent_str}    output.{attr_base}.append({assignment.value})")
                else:
                    # Affectation directe
