...
class RuleConfigModel(BaseModel):
    ...
    output_assignments: Optional[List[OutputAssignmentModel]] = (
        []
...
class RuleflowConfigGenerator:
    ...
    def generate_python_code(self, config: RuleflowConfigModel) -> str:
        ...
        # Méthode decide
        lines.append(
            "    def decide(self, case_input: CaseHandlingDecisionInput) -> CaseHandlingDecisionOutput:",
        )
            ...
            for rule in pkg.rules:
                ...
                for i, code_line in enumerate(rule_code_lines):
                    if i == 0:
                        ...
                        lines.append(
                            f"{rule_indent}{' ' * original_indent}{code_line.strip()}",
                        )
                    else:
                        lines.append("")
    ...
    def validate_config(self, config: RuleflowConfigModel) -> List[str]:
        ...
        for pkg in config.packages:
            ...
            if len(rule_names) != len(set(rule_names)):
                errors.append(
                    f"Les noms de rules doivent être uniques dans le package {pkg.name}",
                )
