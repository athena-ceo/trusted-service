...
class App(ServerApi):
    ...
    def __init__(
        ...
        server_config = ServerConfig.load_from_yaml_file(
            runtime_directory + "/config_server.yaml",
        )
        self.llm_configs: dict[str, LlmConfig] = {
            llm_config.id: llm_config for llm_config in server_config.llm_configs
        }

        app_def_filename = get_app_def_filename(runtime_directory, app_id)
        app_def: AppDef = load_app_def_from_workbook(app_def_filename)

        self.locales: list[SupportedLocale] = [
            cast(SupportedLocale, locale.strip())
            for locale in app_def.locales.split(",")
        ]
        ...
        for decision_engine_config in app_def.decision_engine_configs:
            if decision_engine_config.engine_type == "odm":
                ...
                decision_engine = CaseHandlingDecisionEngineODM(
                    decision_service_url=decision_engine_config.parameter1,
                ...
                module_name, classname = (
                    decision_engine_config.parameter1,
                    decision_engine_config.parameter2,
                )
    ...
    def reload_apps(self) -> NoReturn:
        msg = "reload_apps is handled at the server level, not per-App instance"
        raise NotImplementedError(
            msg,
        )

    def get_app_ids(self) -> list[str]:
        msg = "get_app_ids is handled at the server level, not per-App instance"
        raise NotImplementedError(
            msg,
        )
    ...
    def analyze(
        ...
        return self.localized_apps[locale].analyze(
            app_id,
            locale,
            field_values,
            text,
    ...
    def save_text_analysis_cache(
        ...
        return self.localized_apps[locale].save_text_analysis_cache(
            app_id,
            locale,
            text_analysis_cache,
        )
