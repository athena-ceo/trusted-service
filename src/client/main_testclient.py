...
from src.common.case_model import Case, CaseField, CaseModel
...
def _safe_eval_condition(expr: str) -> bool:
    try:
        ...
        msg = f"Invalid condition syntax: {expr}"
    ...
    def _eval(node):
        ...
        if isinstance(node, ast.Compare):
            ...
            for op, comparator in zip(node.ops, node.comparators):
                ...
                left = right
            return True
        msg = f"Unsupported condition: {expr}"
        raise ValueError(msg)

    result = _eval(tree)
    if not isinstance(result, bool):
        msg = f"Condition did not evaluate to bool: {expr}"
        raise ValueError(msg)
    return result
...
class Context:
    def __init__(
        self, api_client: ApiClient, app_id: str, locale: SupportedLocale
    ) -> None:
        self.api_client: ApiClient = api_client
        self.app_id: str = app_id
        self.locale: SupportedLocale = locale

        self.frontend_localization: ClientLocalization = frontend_localizations[
            locale
        ...
        self.case_handling_detailed_response: CaseHandlingDetailedResponse | None = None
        self.stage = 1

    def update_payload_to_process(self) -> None:
        pass


def add_case_field_input_widget(
    case: Case,
    case_field: CaseField,
    l12n: ClientLocalization,
) -> None:
    ...
    if case_field.type == "date":
        ...
        format_python = (
            case_field.format.replace("DD", "%d")
            .replace("MM", "%m")
            .replace("YYYY", "%Y")
        )
        ...
        if case_field.allowed_values:
            ...
            for option in case_field.allowed_values:
                ...
                try:
                    ...
                    for field_id in case.field_values:
                        ...
                        option_condition = option_condition.replace(
                            "{" + field_id + "}",
                            str(field_value),
                        )
            ...
            def update_case_field_str() -> None:
        ...
        index = None if value is None else 0 if value else 1
        case.field_values[case_field.id] = st.radio(
            label=label,
            options=[True, False],
            index=index,
...
def submit_text_for_ia_analysis() -> None:
    context: Context = st.session_state.context
    analysis_result_and_rendering = context.api_client.analyze(
    ...
    for case_field in context.case_model.case_fields:
        if case_field.extraction in ["EXTRACT", "EXTRACT AND HIGHLIGHT"]:
            context.case.field_values[case_field.id] = analysis_result_and_rendering[
                KEY_ANALYSIS_RESULT
            ][case_field.id]
    st.session_state.context.stage = 2


def submit_case_for_handling() -> None:
    context: Context = st.session_state.context

    payload_str = st.session_state.payload
    payload_dict = json.loads(payload_str)
    case_handling_request = CaseHandlingRequest.model_validate(payload_dict)
    context.case_handling_detailed_response = context.api_client.handle_case(
        context.app_id,
        context.locale,
        case_handling_request,
    )
...
def populate_apps(api_client: ApiClient) -> None:
    ...
    for app_id in app_ids:
        ...
        decision_engine_config_ids: list[str] = (
            api_client.get_decision_engine_config_ids(app_id)
        )
...
def app_selected_unselected() -> None:
...
def locale_selected_unselected() -> None:
    api_client = st.session_state.api_client
    app_proxy = st.session_state.app_proxy
...
def save_cache() -> None:
    ...
    analysis_result: dict[str, Any] = context.analysis_result_and_rendering[
        KEY_ANALYSIS_RESULT
    ]
    text_analysis_cache: str = json.dumps(analysis_result, ensure_ascii=False, indent=4)
    api_client.save_text_analysis_cache(
        context.app_id,
        context.locale,
        text_analysis_cache,
    )
...
def launch_assistant() -> None:
    ...
    st.text_input("Application name")
    st.text_input("Application description")

    # Initialize chat history
    if "messages" not in st.session_state:
        st.session_state.messages = [
            {"role": "assistant", "content": "How many intents should I propose you?"},
        ]
    ...
    if prompt := st.chat_input("Tell me"):
        ...
        response = "I suggest: report_claim, claim_status, billing_issue, policy_change, policy_cancellation"
...
def main_testclient() -> None:
    ...
    with st.sidebar:
        ...
        read_from_cache = st.toggle(
            "Read text analysis from cache",
            value=True,
            key="read_from_cache",
        )
    ...
    with expander_user(l12n.label_request, expanded=True):
        ...
        for case_field in case_model.case_fields:
            ...
            if not case_field.intention_ids and case_field.show_in_ui:
                add_case_field_input_widget(context.case, case_field, l12n)

        st.text_area(
            label=l12n.label_please_describe_your_request,
            height=330,
    ...
    if st.session_state.show_details:
        ...
        highlighted_text = analysis_result_and_rendering[
            KEY_HIGHLIGHTED_TEXT_AND_FEATURES
        ]

        with expander_detail(l12n.label_text_analysis):
            tab_prompt, tab_intents, tab_extraction, tab_misc = st.tabs(
                [
                    l12n.label_prompt,
                    l12n.label_intent_scoring,
                    l12n.label_feature_extraction,
                    l12n.label_misc,
                ],
            )
    ...
    with expander_user(l12n.label_additional_information, expanded=True):
        ...
        labels = [
            scoring["intention_label"] for scoring in scorings if scoring["score"] != 0
        ]

        label_of_selected_intention = st.radio(
            label=l12n.label_confirm_your_request,
            options=labels,
            index=0,
        )
        matching_ids = [
            scoring["intention_id"]
            for scoring in scorings
            if scoring["intention_label"] == label_of_selected_intention
        ]
        if matching_ids:
    ...
    payload = {
        ...
        "highlighted_text_and_features": context.analysis_result_and_rendering[
    ...
    if st.session_state.show_details:
        with expander_detail(l12n.label_processing_of_the_request):
            rendering_email_to_agent, rendering_email_to_requester = (
                case_handling_detailed_response.case_handling_response.case_handling_report
            )
            ...
            handling = (
                case_handling_detailed_response.case_handling_decision_output.handling
            )
            label_task_or_logging = (
                l12n.label_task if handling == "AGENT" else l12n.label_logging
            )
    ...
    st.write(
        case_handling_detailed_response.case_handling_response.acknowledgement_to_requester,
    )
